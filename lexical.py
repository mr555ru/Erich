...
def Scan(inputfile, inputstring, position, pry=False): #pry: подглядывать
    ...
    На входе: inputfile - объект файла
              inputstring - текущая строка
              position - позиция, с которой начинается анализ ("указатель")
              pry - "подглядывает" ли
              
              Использует таблицы токенов (Table_...)
              
    На выходе: tokentype - число, тип нового токена (константы TOKEN_...)
               token_number - номер нового токена в таблице
               inputstring - строка файла, в которой находится указатель после считывания токена
               position - новая позиция "указателя" (если pry=true, то эти два значения не изменятся)
               
               либо None, если достигнут конец файла
    """
...
def mainloop():
    """Тестовый проход, использующийся для проверки модуля."""
...
def print_table(table, name):
    """Вывести на экран таблицу токенов с заголовком name."""
    print "==============%s==============" % name
    for i in xrange(len(table)):
        print "%d\t%s" % (i, table[i])
        
def print_tables():
    """Вывести на экран все таблицы токенов."""
